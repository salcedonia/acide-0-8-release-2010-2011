...
make_dic(ViewNames, Dic, Error) :-    
	my_nf_bagof(SCs,
	           ...
	           (my_member(ViewName,ViewNames),
	            my_view('$des',ViewName,A,B,C,D,SCs)),
	           SCss),
...
instanceMainView(TipoTC, FLt, Dic, V, Ptc, N) :-
     ...
     my_view('$des',V,_Arity,SQLst,_D,_L,_SCs),
     ...
     my_append(Rels, WTables, AllTables),
...
applyTuple(Tc,FLt, Ltables1, sq(exists(select(_, _La, 
                                        from(Ltables2), where(Predicate),
                                        group_by(_), 
                                        having(_H),
                                        order_by([],[])))), Tuple, B) :- 
            cartesian_product(FLt, Ltables2, Lp2), 
            productTupleAll(Tuple, Lp2, Prod),
            my_append(Ltables1, Ltables2, Tab),
            applyE(Tc, FLt, Tab, Predicate, Prod, B, _).
            
applyTuple(Tc,FLt, Ltables1, sq(notExists(select(_La, 
                                    from(Ltables2), where(Predicate),
                                    group_by(_), 
                                    having(_H),
                                    order_by([],[])))), Tuple, B) :- 
...
collectTables(true, []) :- !.
collectTables(false, []) :- !.
collectTables(select(_, _Latt, 
                    from(M), 
                    where(P), 
                    group_by(_), 
                    having(_H),
                    order_by([],[])), L) :- collectTables(P, L1), my_append(M, L1, L), !.
collectTables(and(C1, C2), L) :- !, collectTables(C1, L1), collectTables(C2, L2), my_append(L1, L2, L).
collectTables(or(C1, C2), L) :- !, collectTables(C1, L1), collectTables(C2, L2), my_append(L1, L2, L).
...
% EXISTS: exists((SQ, _))
translatePred(ptc, exists((select(_, L, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_)),
                   LTables, 
                   exists(select(_, L, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond).
      
translatePred(ntc, exists((select(_, L, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_)),
                   LTables, 
                   notExists(select(_, L, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond).
      
% NOT EXISTS : not(exists((SQ, _)))                                              
translatePred(ptc, not(exists((select(_, L, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_))),
                   LTables, 
                   notExists(select(_, L, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond).  
      
translatePred(ntc, not(exists((select(_, L, from(M), where(C), group_by(Attr), having(Hc), order_by(_,_)),_))),
                   LTables, 
                   exists(select(_, L, from(M), where(Cond), group_by(Attr), having(Hc), order_by(_,_)))) :- 
      translatePred(ptc, C, LTables, Cond). 
...
translatePred(ptc, AnyCond,
           LTables, 
           exists(select(_, all, from(M), where(and(Cond,bc(Ro1, Op, Ro2))), 
           group_by(AttGb), having(Pred_h),order_by(Oby1, Oby2)))) :-
           
      AnyCond=..Res, Res=[Type, E1, (SQ, _ )],  mySqType(Type, Op, any),
      SQ = select(_, [E2], from(M), where(C), group_by(AttGb), having(Pred_h),order_by(Oby1, Oby2)),         
...
translatePred(ptc, AllCond,                        
              LTables, 
            notExists(select(_, Ro2, from(M), where(and(Cond,bc(Ro1,NOp,Ro2) )),
                      group_by(AttGb), having(Pred_h),order_by(Oby1, Oby2) ))) :- 
