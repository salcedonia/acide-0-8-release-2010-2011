package gui.menu.configuration.lexicon.gui;

import gui.mainWindow.MainWindow;
import gui.menu.configuration.lexicon.utils.AcideTableModel;
import gui.menu.configuration.lexicon.utils.AcideTableSorter;
...
public class LexiconConfigurationWindow extends JFrame {
	...
	public LexiconConfigurationWindow() {
		...
		// Updates the log
		Log.getLog().info(_labels.getString("s376"));
		...
		_addButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				// Updates the log
				Log.getLog().info(_labels.getString("s417") + _wordTextField.getText());
			}
		});
		...
		_quitButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				if (row != -1) {
					...
					// Updates the log
					Log.getLog().info(_labels.getString("s419") + value);
				}
			}
		});
		...
		_applyButton.addActionListener(new ActionListener() {		
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				int editor = MainWindow.getInstance().getEditorManager()
						.getNumEditors();
		...
		// Updates the log
		Log.getLog().info(_labels.getString("s420"));
	...
	public void buildColorPaletteWindow() {
		...
		// Updates the log
		Log.getLog().info(_labels.getString("s407"));
		...
		// Updates the log
		Log.getLog().info(_labels.getString("s412"));
	}
}
