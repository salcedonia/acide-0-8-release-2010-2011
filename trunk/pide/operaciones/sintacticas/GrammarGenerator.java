package operaciones.sintacticas;
...
public class GrammarGenerator {
	...
	// FIXME No se carga el idioma que se deja puesto
	...
	public static boolean generate(/*String javacPath,*/ String grammarName) throws Exception {
...
//		}
		...
		try {
			...
			//e.printStackTrace();
			logger.error(labels.getString("s245"));
			generated = false;
		}
		ModifyParser();
		//while(!f1.exists());
...
//		f.delete();
	...
	private static void ModifyParser() {
		...
		String txt = null;
		txt = f.cargar("GrammarParser.java");
		...
		f.salvar("GrammarParser.java",txt);
	}
	...
	private static boolean reallocateFile(String source, String target) {
		...
		try {
			...
			logger.error(labels.getString("s210") + ": " + labels.getString("s211"));
		}
...
//		boolean saved = f.salvar(target,sourceText);
		...
		/*if(saved)*/ logger.info(labels.getString("s212") + target);
		//else logger.error(labels.getString("s213") + target);
		boolean deleted = sourceFile.delete();
		//System.out.println("deleted: " + deleted);
		if(deleted) logger.info(labels.getString("s214") + source);
		else logger.error(labels.getString("s215") + source);
...
//		boolean copied = f.salvar(target,sourceText);
		...
		/*if(copied)*/ logger.info(labels.getString("s212") + target);
		//else logger.error(labels.getString("s213") + target);
		return copied;
	}
