...
public class ExternalCommandGUI {
	...
	public ExternalCommandGUI() {
		...
		// EXEC PATH
		_execPathLabel = new JLabel(_labels.getString("s345"),
				JLabel.CENTER);
		...
		// EXEC
		_execLabel = new JLabel(_labels.getString("s346"), JLabel.CENTER);
			...
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (_execPathTextField.isEnabled()) {
						execPath = _execPathTextField.getText();
					} else {
						String calculedPath = "";
						String saux = _execTextField.getText();
					...
					String commandAux = _execTextField.getText();
					...
					// IF THE PROJECT CONFIGURATION IS DEFAULT CONFIGURATION
