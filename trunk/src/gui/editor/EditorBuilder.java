		...
		class TestPlafLayout extends TabbedPaneLayout {
			...
			class CloseButtonAction extends AbstractAction {
				...
				public void actionPerformed(ActionEvent e) {
					...
					if (mainWindow.getEditorBuilder().isRedButton(_index)) {
						...
						if (chosenOption == JOptionPane.OK_OPTION) {
							...
							// IF NEW FILE
							if (mainWindow.getEditorBuilder().getEditorAt(
								...
								if (!filePath.equals(" ")) {
									...
									if (savingResult) {
										...
										mainWindow.getEditorBuilder()
												.getEditorAt(_index).setAbsolutePath(
														filePath);
							...
							// SET IS OPENED TO FALSE TO ALL THE FILES IN THE
							// CONFIGURATION
							for (int filPos = 0; filPos < mainWindow
						...
						for (int filePos = 0; filePos < mainWindow
							...
							if (mainWindow.getProjectConfiguration().getFileAt(
								...
								// SET OPENED TO FALSE TO ALL THE FILES OF THE
								// CONFIGURATION
					...
					for (int pos = _index; pos < _closeButtons.size() - 1; pos++) {
						...
						closeButton.setForeground(nextCloseButton
								.getForeground());
	...
	class EditorBuilderMouseListener extends MouseAdapter {
		...
		public void mouseClicked(MouseEvent arg0) {
			...
			if (_tabbedPane.getComponentCount() != 0) {
				...
				String selectedEditorPath = getSelectedEditor().getAbsolutePath();
			...
			if (!project.equals("./configuration/default.acidePrj")) {
				...
				for (int i = 0; i < MainWindow.getInstance().getProjectConfiguration()
					...
					if (MainWindow.getInstance().getProjectConfiguration().getFileAt(i)
						...
						for (int j = 0; j < MainWindow.getInstance()
							...
							if (MainWindow.getInstance().getProjectConfiguration().getFileAt(
								...
								MainWindow.getInstance().getProjectConfiguration().getFileAt(
										j).setIsMainFile(false);
								
								// SET COMPILABLE TO FALSE
								MainWindow.getInstance().getProjectConfiguration().getFileAt(
										j).setIsCompilableFile(false);
						...
						MainWindow.getInstance().getProjectConfiguration().getFileAt(i)
								.setIsMainFile(true);
						MainWindow.getInstance().getProjectConfiguration().getFileAt(i)
