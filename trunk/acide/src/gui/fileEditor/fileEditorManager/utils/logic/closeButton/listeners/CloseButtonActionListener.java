...
public class CloseButtonActionListener extends AbstractAction {
	...
	public void actionPerformed(ActionEvent actionEvent) {
					...
					if (!filePath.equals(" ")) {
						...
						boolean savingResult = textFile.save(filePath,
								MainWindow.getInstance().getFileEditorManager()
										.getFileEditorPanelAt(_index).getTextEditionAreaContent());
		...
		// No more tabs?
		if (MainWindow.getInstance().getFileEditorManager().getTabbedPane().getTabCount() == 0) {
		...
		for (int pos = _index; pos < MainWindow.getInstance()
				.getFileEditorManager().getTestPlaf().getCloseButtons().size() - 1; pos++) {
