...
/**
 * Project menu of the application.
 * 
	...
	class NewProjectListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (int z = numEditors - 1; z >= 0; z--) {
				MainWindow.getInstance().getEditorBuilder().setSelectedEditorAt(z);
			...
			if (!cancelSelected)
				MainWindow.getInstance().setProjectGUI(GUIFactory.getInstance().buildProjectGUI());
		}
	}

	/**
	 * Save project item listener.
	 * 
	 * @project ACIDE - A Configurable IDE (c).
	 * @version 0.8.
	 */
	...
	class OpenProjectListener implements ActionListener {
		...
		public void load(String file) {
			...
			mainWindow.setSize(mainWindow.getProjectConfiguration()
					...
					.getWindowHeight());
			mainWindow.setLocation(mainWindow.getProjectConfiguration()
					.getPosX(), mainWindow.getProjectConfiguration().getPosY());
			...
			for (int j = 0; j < mainWindow.getProjectConfiguration()
				...
				if (!mainWindow.getProjectConfiguration().getFileAt(j)
					...
					if (mainWindow.getProjectConfiguration().getFileAt(j)
						...
						mainWindow.getEditorBuilder().newTab(name2, file2,
								text, true, type);
						mainWindow.validate();
						mainWindow.repaint();
						...
						int numEditor = mainWindow.getEditorBuilder()
								.getSelectedEditorIndex();
						DefaultStyledDocument doc = mainWindow
			...
			// LOAD LANGUAGE
			String language = mainWindow.getProjectConfiguration()
					.getLanguage();
	...
	class CloseProjectListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent arg0) {
			...
			if (!cancelSelected) {
				...
				mainWindow.getExplorer().getTreeModel().reload();
				mainWindow.getExplorer().getPopupMenu().getAddFile()
						.setEnabled(false);
				...
				mainWindow.getMenu().getFile().getCloseAllFiles().setEnabled(
						true);
				...
				if (currentSelection != null) {
					...
					if (!p.isDirectory()) {
						...
						if (parent != null) {
							...
							mainWindow.getProjectConfiguration().removeFileAt(
									cont);
							mainWindow.getStatusBar().setMessage("");
							int editor = -1;
							...
							// Close
							mainWindow.getEditorBuilder().getPane().remove(
									editor);
				...
				if (currentSelection != null) {
					...
					if (!p.isDirectory()) {
						...
						if (parent != null) {
							...
							ExplorerFile f = mainWindow
									.getProjectConfiguration().getFileAt(cont);
						...
						for (int i = 0; i < MainWindow.getInstance()
							...
							if (MainWindow.getInstance()
								...
								for (int j = 0; j < MainWindow.getInstance()
										.getEditorBuilder().getNumEditors(); j++) {
	...
	class SetCompilableListener implements ActionListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (path != null) {
				...
				// NOT COMPILABLE OR COMPILABLE AND MAIN
			...
			if (path != null) {
				...
				if (explorerFile.isCompilableFile()
					...
					if (!explorerFile.isDirectory()) {
						...
						// REMOVES THE ICON IN THE EDITOR
						for (int j = 0; j < MainWindow.getInstance()
								.getEditorBuilder().getNumEditors(); j++) {
							if (MainWindow.getInstance().getEditorBuilder()
									.getEditorAt(j).getAbsolutePath().equals(
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				if (!MainWindow.getInstance().getProjectConfiguration()
					...
					// ADD THE EXTENSION IF THE NAME DOESN'T CONTAIN IT
					...
					// IS THE FIRST TIME THAT A PROJECT HAS BEEN SAVED
					MainWindow.getInstance().getProjectConfiguration()
							.setFirstSave(true);
					PropertiesManager.setProperty("defaultAcideProject", file);
