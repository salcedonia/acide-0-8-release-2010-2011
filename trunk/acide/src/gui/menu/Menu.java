...
class MouseClickMenu extends MouseAdapter {
	...
	public void mousePressed(MouseEvent arg0) {
				...
				if (editor > 0) {
					...
					if (!mainWindow.getEditorBuilder().getSelectedEditor()
							...
							|| (mainWindow.getEditorBuilder()
									...
									.getEditorBuilder().getSelectedEditor()
									.isMainFile()))
						mainWindow.getMenu().getFile().getSetCompilable()
								.setEnabled(true);
					if (mainWindow.getEditorBuilder().getSelectedEditor()
							.isCompilerFile()
							&& !mainWindow.getEditorBuilder()
									.getSelectedEditor().isMainFile())
						mainWindow.getMenu().getFile().getUnsetCompilable()
								.setEnabled(true);
				}
			}
		...
		if (mainWindow.getMenu().getFile().isSelected()) {
			...
			mainWindow.getMenu().getFile().getSetMain().setEnabled(false);
			mainWindow.getMenu().getFile().getUnsetMain().setEnabled(false);
			mainWindow.getMenu().getFile().getSetCompilable().setEnabled(false);
			mainWindow.getMenu().getFile().getUnsetCompilable().setEnabled(false);
		...
		if (mainWindow.getMenu().getEdit().isSelected()) {
			...
			if (Toolkit.getDefaultToolkit().getSystemClipboard()
				...
				if (!mainWindow.getOutput().getOutput().hasFocus())
					mainWindow.getMenu().getEdit().getPaste().setEnabled(true);
				else if (mainWindow.getOutput().getOutput().getSelectionStart() >= mainWindow
			...
			if (editor > 0) {
				if (mainWindow.getOutput().getOutput().hasFocus()
						&& mainWindow.getOutput().getOutput().getSelectedText() != null) {
					mainWindow.getMenu().getEdit().getCopy().setEnabled(true);
					if (mainWindow.getOutput().getOutput().getSelectionStart() >= mainWindow
							.getOutput().getMaxCaretPosition())
				...
				if (mainWindow.getOutput().getOutput().getSelectedText() != null) {
					mainWindow.getMenu().getEdit().getCopy().setEnabled(true);
					if (mainWindow.getOutput().getOutput().getSelectionStart() >= mainWindow
							.getOutput().getMaxCaretPosition())
						mainWindow.getMenu().getEdit().getCut().setEnabled(true);
		...
		if (mainWindow.getOutput().getOutput().isFocusOwner()) {
