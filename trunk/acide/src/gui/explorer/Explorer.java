...
public class Explorer extends JPanel {
	 ...
	 */
	private ExplorerPopupMenu _popup;
	/**
	 * Explorer size.
	 */
	 ...
	 */
	public ExplorerPopupMenu getPopupMenu(){
		return _popup;
	}
			...
			if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
				...
				if (path != null) {
					...
					if (!fc.isDirectory()) {
						...
						TextFile fd = new TextFile();
						EditorBuilder ce = MainWindow.getInstance().getEditorBuilder();
						String s = null;
						s = fd.load(fc.getPath());

						int j = -1;
						for (int z = 0; z < MainWindow.getInstance().getProjectConfiguration()
								.getNumFilesFromList(); z++) {
							if (MainWindow.getInstance().getProjectConfiguration().getFileAt(z)
									.getPath().equals(fc.getPath()))
								j = z;
						}
						...
						// COMPILABLE FILE
						if (MainWindow.getInstance().getProjectConfiguration().getFileAt(j).isCompilableFile()) {
							t = 2;
						...
						// MAIN FILE
						if (MainWindow.getInstance().getProjectConfiguration().getFileAt(j).isMainFile()) {
							t = 1;
							MainWindow.getInstance().getStatusBar().setMessage(filePath + " <MAIN>");
						}

						ce.newTab(fc.getPath(), fc.getPath(), s, true, t);
						
						// UNDO REDO
						MainWindow.getInstance().getMenu().getFile().enableSaveFileAs();
						int numeditor = MainWindow.getInstance().getEditorBuilder()
								.getSelectedEditorIndex();
						Document doc = MainWindow.getInstance().getEditorBuilder()
								.getEditorAt(numeditor).getEditor()
								.getDocument();
						doc.addUndoableEditListener(new UndoableEditListener() {
						...
						MainWindow.getInstance().getEditorBuilder().getEditorAt(numeditor).getEditor()
								.setCaretPosition(0);
					}
				}
			}
