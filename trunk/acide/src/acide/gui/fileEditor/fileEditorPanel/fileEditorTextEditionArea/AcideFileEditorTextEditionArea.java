	...
	protected void buildTextPane(AcideStyledDocument styledDocument) {
		...
		_textComponent = new AcideTextComponent(styledDocument) {
			...
			public boolean getScrollableTracksViewportWidth() {
				...
				// Enables or disables the line wrapping
		...
		// Assign the CONTROL+Z to the text component
		_textComponent.registerKeyboardAction(AcideMainWindow.getInstance()
				.getMenu().getEditMenu().getUndoAction(),
				KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.CTRL_MASK),
				JComponent.WHEN_FOCUSED);

		// Assign the CONTROL+Y to the text component
		_textComponent.registerKeyboardAction(AcideMainWindow.getInstance()
				.getMenu().getEditMenu().getRedoAction(),
				KeyStroke.getKeyStroke(KeyEvent.VK_Y, InputEvent.CTRL_MASK),
				JComponent.WHEN_FOCUSED);

		// Removes the CONTROL+A key stroke in order to not overlap the
		// CONTROL+A from the menu
		_textComponent.getInputMap().put(
