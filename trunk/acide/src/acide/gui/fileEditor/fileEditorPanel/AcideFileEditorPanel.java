...
public class AcideFileEditorPanel extends JPanel {
	 ...
	 */
	public AcideFileEditorPanel(String filePath, String fileContent,
			boolean isEditable, long lastChange, long lastSize,
	...
	public String getTextEditionAreaContent() {
		...
		int length = _textEditionAreaList.get(_activeTextEditionAreaIndex)
				.getTextComponent().getDocument().getLength();

		try {
			return _textEditionAreaList.get(_activeTextEditionAreaIndex)
					.getTextComponent().getDocument().getText(0, length);
	 ...
	 * 
	 * <p>
	 * The replacement of all the "\r\n" by "\n" avoids problems in the
	 * comparisons between the file disk copy and the JTextPane document text.
	 * </p>
	 * 
	 * @param fileDiskCopy
	 *            new value to set.
	...
	public void setFileDiskCopy(String fileDiskCopy) {
		...
		_fileDiskCopy = "";
		_fileDiskCopy = _fileDiskCopy + fileDiskCopy.replaceAll("\r\n", "\n");
	}
