...
public class AcideLanguageMenu extends JMenu {
	...
	public void changeLanguage(String selectedLanguage) {
		...
		try {
			...
			// Gets the ACIDE - A Configuration IDE current grammar configuration
			String currentGrammarConfiguration = AcideResourceManager.getInstance()
					.getProperty("currentGrammarConfiguration");
			
			// Gets the grammar name
			int index = currentGrammarConfiguration.lastIndexOf("\\");
			if (index == -1)
				index = currentGrammarConfiguration.lastIndexOf("/");
			String grammarName = currentGrammarConfiguration.substring(index + 1,
