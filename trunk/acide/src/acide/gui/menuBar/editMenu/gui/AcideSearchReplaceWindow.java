...
public class AcideSearchReplaceWindow extends JFrame {
	...
	private void setListeners() {
		...
		_searchTextField.registerKeyboardAction(new SearchButtonListener(),
	...
	private void setSearchWindowConfiguration() {
		...
		// Hides the respect capitalization check box
		_respectCapitalizationCheckBox.setVisible(false);
	...
	private void setReplaceWindowConfiguration() {
		...
		// Shows the respect capitalization check box
		_respectCapitalizationCheckBox.setVisible(true);
			...
			if (resultPosition != -1) {
				...
				if (searchDirection == AcideSearchDirection.FORWARD) {
					...
					if (_currentCheckedEditorIndex >= AcideMainWindow
						...
						// Performs the search
						allDocumentsSearchOrReplace(temporalDirection,
								isReplace, replaceText);
					}
				}
