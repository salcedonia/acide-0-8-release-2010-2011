...
public class AcideReplaceWindow extends JFrame {
	...
	private void setListeners() {
		...
		AcideSearchReplaceWindowKeyboardListener keyListener = new AcideSearchReplaceWindowKeyboardListener();

		// Sets the search text field key listener
		_searchTextField.addKeyListener(keyListener);

		// Sets the case sensitive check box key listener
		_caseSensitiveCheckBox.addKeyListener(keyListener);

		// Sets the regular expressions check box key listener
		_regularExpressionsCheckBox.addKeyListener(keyListener);

		// Sets the complete words check box key listener
		_completeWordsCheckBox.addKeyListener(keyListener);

		// Sets the forward radio button key listener
		_forwardRadioButton.addKeyListener(keyListener);

		// Sets the backward radio button key listener
		_backwardRadioButton.addKeyListener(keyListener);

		// Sets the both directions radio button key listener
		_bothDirectionsRadioButton.addKeyListener(keyListener);

		// Sets the selected text radio button key listener
		_selectedTextRadioButton.addKeyListener(keyListener);

		// Sets the all documents radio button key listener
		_allDocumentsRadioButton.addKeyListener(keyListener);

		// Sets the current document radio button key listener
		_currentDocumentRadioButton.addKeyListener(keyListener);
	}
	...
	private String respectCapitalization() {
		...
		String[] replacedWords = _searchTextField.getText().split(" ");

		// Gets the separate words to replace
		String[] replaceWords = _searchTextField.getText().split(" ");
	...
	class ReplaceButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			if (_respectCapitalizationCheckBox.isSelected())
				...
				_replaceText = "" + respectCapitalization();
