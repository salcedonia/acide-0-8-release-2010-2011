...
public class AcideNewProjectConfigurationWindow extends JFrame {
	...
	public void updateExplorerPanel() {
		...
		for (int index = 0; index < AcideMainWindow.getInstance()
			...
			if (!AcideMainWindow.getInstance().getFileEditorManager()
				...
				newProjectFile.setIsCompilableFile(AcideMainWindow.getInstance()
						.getFileEditorManager().getFileEditorPanelAt(index)
						.isCompilableFile());
		...
		AcideMainWindow.getInstance().getMenu().getFileMenu().getOpenAllFilesMenuItem()
				.doClick();
	}
	...
	public void updateToolBarConfiguration() {
		...
		try {
			// Gets the ACIDE - A Configurable IDE current tool bar configuration
	...
	public void updateMainWindowConfiguration() {
		...
		AcideWindowConfiguration.getInstance()
				.setVerticalSplitPaneDividerLocation(
						AcideMainWindow.getInstance().getExplorerPanel().getWidth());

		// Sets the horizontal split pane divider location
		AcideWindowConfiguration.getInstance()
				.setHorizontalSplitPaneDividerLocation(
						AcideMainWindow.getInstance().getConsolePanel().getHeight());
		...
		// popup menu
		AcideMainWindow.getInstance().getExplorerPanel().getPopupMenu().getAddFile()
				.setEnabled(true);
	...
	private void updateGrammarConfiguration() {
		...
		// Updates the ACIDE - A Configurable IDE current grammar configuration
		AcideResourceManager.getInstance().setProperty("currentGrammarConfiguration",
		...
		AcideMainWindow.getInstance().getMenu().getConfigurationMenu().getGrammarMenu()
				.getSaveGrammarMenuItem().setEnabled(false);
	}
	...
	class LoadLexiconButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			AcideMainWindow.getInstance().getMenu().getConfigurationMenu().getLexiconMenu()
					.getLoadLexiconMenuItem().doClick();
	...
	class LoadGrammarButtonAction implements ActionListener {
		...
		public void actionPerformed(ActionEvent actionEvent) {
			...
			// Loads the grammar
			AcideMainWindow.getInstance().getMenu().getConfigurationMenu().getGrammarMenu()
					.getLoadGrammarMenuItem().doClick();
