		...
		try {
			...
			AcideFileManager
					...
					.addChoosableFileFilter(
							new AcideFileExtensionFilterManager(askExtension,
									AcideLanguageManager.getInstance()
											.getLabels().getString("s1036")));

			// Ask to the user for the file
			String[] filePaths = AcideFileManager.getInstance()
					.askMultipleAbsolutePath();
		...
		if (!isOpened) {
			...
			AcideMainWindow
					...
					.updatesTabbedPane(projectFile.getAbsolutePath(),
							fileManager.load(projectFile.getAbsolutePath()),
							true, projectFileType, 0, 0, 1);
