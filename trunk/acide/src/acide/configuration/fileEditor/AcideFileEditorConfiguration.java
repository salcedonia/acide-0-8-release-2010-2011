	...
	public boolean save() {
		...
		if (selectedFileEditorPanelIndex != -1) {
			...
			for (int index = 0; index < AcideMainWindow.getInstance()
				...
				AcideFileEditorPanel fileEditorPanel = AcideMainWindow.getInstance()
				...
				fileEditorPanelConfiguration
				.setActiveTextEditionArea(fileEditorPanel.getActiveTextEditionAreaIndex());
			...
			// FOREGROUND COLOR
			_foregroundColor = AcideMainWindow.getInstance().getFileEditorManager()
					.getSelectedFileEditorPanel().getActiveTextEditionArea()
					.getForeground();

			// BACKGROUND COLOR
			_backgroundColor = AcideMainWindow.getInstance().getFileEditorManager()
					.getSelectedFileEditorPanel().getActiveTextEditionArea()
					.getBackground();
