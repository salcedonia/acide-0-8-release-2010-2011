...
import acide.gui.savingResourcesWindow.AcideSavingResourcesWindow;
...
public class AcideWorkbenchManager {
			...
			if (!file.isDirectory() && file.exists()) {
				...
				AcideMainWindow
						...
						.updatesTabbedPane(
								...
								fileType,
								AcideFileEditorConfiguration.getInstance()
										.getFileAt(index1).getCaretPosition(),
								AcideFileEditorConfiguration.getInstance()
										.getFileAt(index1)
	...
	public static void loadGrammarWorkbenchConfiguration(ResourceBundle labels) {
		...
		try {
			...
			grammarName = currentGrammarConfiguration.substring(
					lastIndexOfSlash + 1,
	...
	public static void loadMainWindowConfiguration() {
		...
		try {
			...
			// Loads the window configuration
	...
	public static AcideLexiconConfiguration loadLexiconWorkbenchConfiguration() {
		...
		try {
			...
			lexiconConfiguration.load(AcideResourceManager.getInstance()
					.getProperty("lexiconConfiguration"));
		} catch (Exception exception) {
			...
			if (AcideProjectConfiguration.getInstance().isDefaultProject()) {
				...
				if (AcideProjectConfiguration.getInstance()
					...
					AcideMainWindow.getInstance().getExplorerPanel()
							.getPopupMenu().getRemoveFile().setEnabled(true);
				else
					// Removing files in the EXPLORER menu is not allowed
					AcideMainWindow.getInstance().getExplorerPanel()
							.getPopupMenu().getRemoveFile().setEnabled(false);
	...
	private int getNewFileIndex() {
		...
		int numberOfEditorPanels = AcideMainWindow.getInstance()
						...
						if (numberOfFileEditorPanels > 0) {
							...
							for (int index = numberOfFileEditorPanels - 1; index >= 0; index--) {
								...
								if (AcideMainWindow.getInstance()
									...
									resourceList.add(AcideMainWindow
											.getInstance()
											.getFileEditorManager()
	...
	public void saveRestOfWorkbenchConfiguration() {
		...
		try {
			...
			if ((currentGrammarConfiguration.endsWith("lastModified.jar"))
					|| (currentGrammarConfiguration.endsWith("newGrammar.jar"))) {
