...
public class GrammarConfigurationWindow extends JFrame {
	...
	public GrammarConfigurationWindow(boolean modify) {
		...
		if(modify){
			...
			try {
				currentGrammar = ResourceManager.getInstance().getProperty("currentGrammar");
			} catch (Exception exception) {
		...
		_acceptButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent actionEvent) {
				...
				try {
					...
					String previousGrammar = ResourceManager
							.getInstance().getProperty("currentGrammar");
					
					if (_changesAreSaved)
						// Updates the RESOURCE MANAGER
						ResourceManager.getInstance().setProperty("previousGrammar",
								previousGrammar);
		...
		if (modify){
			...
			try {
				...
				// GET THE PATH
				currentPath = ResourceManager.getInstance().getProperty("currentGrammar");
	...
	public static void saveGrammarGUI() {
		...
		try {
			String previous = ResourceManager.getInstance().getProperty("previousGrammar");
			String current = ResourceManager.getInstance().getProperty("currentGrammar");
			...
			// Updates the RESOURCE MANAGER
			ResourceManager.getInstance().setProperty("currentGrammar", previous);
	...
	public static void saveAsGrammarGUI() {
		...
		try {
			...
			if (optionChosen == JFileChooser.APPROVE_OPTION) {
				...
				// Updates the RESOURCE MANAGER
				ResourceManager.getInstance().setProperty("currentGrammar", fileName);
				...
				MainWindow.getInstance().getStatusBar().setGrammarMessage(
						labels.getString("s248") + " " + grammarName);
				MainWindow.getInstance().getProjectConfiguration().setSyntacticConfiguration(
						fileName);
				_changesAreSaved = true;
